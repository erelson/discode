...
export function getGeminiHookSourcePath(filename?: string): string {
  ...
  const candidates = [
    join(import.meta.dirname, '../../gemini', 'hook', hookFile),   // source layout: src/agents/gemini/ → src/gemini/hook
    join(import.meta.dirname, 'hook', hookFile),             // original layout: src/gemini/
